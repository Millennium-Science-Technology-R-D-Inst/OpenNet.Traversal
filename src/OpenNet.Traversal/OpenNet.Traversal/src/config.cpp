...
namespace opennet
{
	config parse_config(int argc, char** argv)
	{
		...
		for (int index = 1; index < argc; ++index)
		{
			...
			else if (option == "--advertise-hostname")
				result.advertised_hostname = value;
		...
		if (!result.advertised_hostname.empty()
			&& (!result.advertised_address.empty()
				|| !result.advertised_ipv6_address.empty()))
		{
			throw std::invalid_argument(
				"--advertise-hostname cannot be combined with --advertise or --advertise-ipv6");
		}
		if (result.advertised_hostname.empty() && result.advertised_address.empty())
		...
		if (result.advertised_hostname.empty()
			&& result.advertised_address == "0.0.0.0")
		{
			throw std::invalid_argument(
	...
	std::string usage()
	{
		return
			...
			"  --advertise-hostname <hostname>  periodically resolve public A/AAAA records\n"
